...
export function CompanyDetailView({ client, integrations, reports }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* Back + Header */}
      <div>
        <Link href="/companies" className="inline-flex items-center gap-1.5 text-sm text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)] transition-colors mb-4">
          <ArrowLeft className="w-4 h-4" /> Todos os empresas
        </Link>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="sm:ml-auto flex gap-2">
            <Link href={`/companies/${client.id}/integrations`} className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-[var(--color-foreground)] bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl hover:bg-[var(--color-surface)] transition-colors">
              <Plug className="w-4 h-4" /> Integrações
            </Link>
            <Link href={`/companies/${client.id}/edit`} className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-[var(--color-primary)] rounded-xl hover:opacity-90 transition-all glow-primary">
        ...
        >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  { label: 'Empresa desde', value: new Intl.DateTimeFormat('pt-BR', { month: 'short', year: 'numeric' }).format(new Date(client.createdAt)), sub: 'Data de cadastro', icon: Clock, color: 'text-amber-400', bg: 'bg-amber-500/10' },
              ...
              {reports.length > 0 && (
                <div className="card border border-[var(--color-border)] overflow-hidden">
                  <div className="px-5 py-4 border-b border-[var(--color-border)] flex items-center justify-between">
                    ...
                    <Link href={`/reports?companyId=${client.id}`} className="text-xs text-[var(--color-primary)] hover:underline">Ver todos</Link>
                  </div>
          ...
          {activeTab === 'integrations' && (
            <div className="space-y-4">
              <div className="flex justify-end">
                <Link
                  href={`/companies/${client.id}/integrations`}
              ...
              {integrations.length === 0 ? (
                <div className="card border border-[var(--color-border)] py-16 text-center">
                  ...
                  <Link href={`/companies/${client.id}/integrations`} className="inline-flex items-center gap-2 mt-4 px-4 py-2 text-sm font-medium text-white bg-[var(--color-primary)] rounded-xl hover:opacity-90">
          ...
          {activeTab === 'reports' && (
            <div className="space-y-4">
              <div className="flex justify-end">
                <Link
                  href={`/reports/new?companyId=${client.id}`}
                  className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-[var(--color-primary)] rounded-xl hover:opacity-90 transition-all"
                >
